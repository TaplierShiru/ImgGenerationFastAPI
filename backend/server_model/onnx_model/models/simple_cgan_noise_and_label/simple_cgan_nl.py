...
class ModelSimpleCGANNL(BaseModel):
    ...
    def predict(self, label: int, **kwargs) -> Image:
        ort_inputs = {
            self._ort_session.get_inputs()[0].name: np.random.normal(
                size=(1, self._config_data['noise_dim'])
            ).astype(np.float32),
            self._ort_session.get_inputs()[1].name: np.array([[label]], dtype=np.int64),
        }
